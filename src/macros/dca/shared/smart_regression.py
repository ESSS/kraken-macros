...
class Regression:
    ...
    def ExponentialFit(self, time_array_in_years, opr_history):
        
        def func(x, a, b):
            return a * np.exp(-b * x)
        
        bissection = self.SmartIntervalSearcher(opr_history)
    ...
    def WCFitCoeff(self, wc, opt):
        
        if np.sum(wc) < 0.01:
            slope = 0
            intercept = 0
        else:            
            position = bisect.bisect_left(wc,0.001)
            new_wc, new_opt = np.array(wc[position:]), np.array(opt[position:]) * 0.0062898
            slope, intercept, r_value, p_value, slope_std_error = stats.linregress(new_opt, new_wc)
            
            if r_value < 0.99:
                ratio = float(len(new_wc)) / float(len(wc))
                
                if ratio > 0.1:
                    
                    if ratio/2 > 0.1:
                        position = position + int(len(new_wc)/2)
                        new_wc, new_opt = np.array(wc[position:]), np.array(opt[position:]) * 0.0062898
                        slope, intercept, r_value, p_value, slope_std_error = stats.linregress(new_opt, new_wc)
                        
                        if r_value < 0.99:
                            position = int(0.9*len(wc))
                            new_wc, new_opt = np.array(wc[position:]), np.array(opt[position:]) * 0.0062898
                            slope, intercept, r_value, p_value, slope_std_error = stats.linregress(new_opt, new_wc)
                        
        return slope, intercept
    
    def WORFitCoeff(self, wor, opt):
        ...
        if np.sum(wor) < 0.01:
            ...
            position = bisect.bisect_left(wor,0.001)
            ...
            popt, pcov = curve_fit(func, new_opt, new_wor)
        
        return popt.tolist()
       
    def SmartIntervalSearcher(self, curve_to_be_analysed):
